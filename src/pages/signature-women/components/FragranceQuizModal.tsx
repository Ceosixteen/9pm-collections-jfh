...
export const FragranceQuizModal: React.FC<FragranceQuizModalProps> = ({
  ...
  const handleCalculateResult = (vibeChoice: string) => {
    ...
    if (vibeChoice === 'plum_oud') {
      match = perfumes.find((p) => p.id === 'hawas-black');
    } else if (vibeChoice === 'leather_spice') {
      match = perfumes.find((p) => p.id === 'hawas-fire');
    } else if (vibeChoice === 'fresh_citrus') {
      match = perfumes.find((p) => p.id === 'hawas-ice');
    } else if (vibeChoice === 'floral_musk') {
      match = perfumes.find((p) => p.id === 'hawas-pink');
    } else {
      match = perfumes.find((p) => p.id === 'hawas-for-him');
    }
        ...
        {step === 2 && (
          <div className="space-y-4 animate-fadeIn">
            ...
            <div className="space-y-2">
              ...
              >
                <span className="font-bold text-slate-900 block text-xs sm:text-sm">Dark Berries, Lavender & Smoky Vetiver</span>
                <span className="text-[11px] text-slate-500">Mysterious, dark aromatic woody nocturnal vibe.</span>
              </button>
              ...
              <button
                ...
                className="w-full p-3.5 rounded-2xl bg-slate-50 border border-gray-200 hover:border-purple-300 text-left transition-all cursor-pointer"
              >
                <span className="font-bold text-slate-900 block text-xs sm:text-sm">Blood Orange, Chili & Molten Leather</span>
                <span className="text-[11px] text-slate-500">Hot, rich, and intoxicatingly warm for gentlemen.</span>
              </button>
              ...
              <button
                ...
                className="w-full p-3.5 rounded-2xl bg-slate-50 border border-gray-200 hover:border-purple-300 text-left transition-all cursor-pointer"
              >
                <span className="font-bold text-slate-900 block text-xs sm:text-sm">Apple, Cinnamon & Legendary Ambergris</span>
                <span className="text-[11px] text-slate-500">The iconic Signature compliment magnet, beast mode.</span>
              </button>
              ...
              <button
                ...
                className="w-full p-3.5 rounded-2xl bg-slate-50 border border-gray-200 hover:border-purple-300 text-left transition-all cursor-pointer"
              >
                <span className="font-bold text-slate-900 block text-xs sm:text-sm">Icy Apple, Mint & Frozen Lemon</span>
                <span className="text-[11px] text-slate-500">Crisp arctic aquatic citrus energizer.</span>
              </button>
              ...
              <button
                ...
                className="w-full p-3.5 rounded-2xl bg-slate-50 border border-gray-200 hover:border-purple-300 text-left transition-all cursor-pointer"
              >
                <span className="font-bold text-slate-900 block text-xs sm:text-sm">Sambac Jasmine, Iris & Praline Musk</span>
                <span className="text-[11px] text-slate-500">Elegant feminine sweetness and grace.</span>
              </button>
            </div>
          </div>
        )}
