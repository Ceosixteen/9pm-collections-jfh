...
export const Hero: React.FC<HeroProps> = ({
  ...
  // Select top featured items from the Signature Fragrances for Women for the right stack
  const pForHim = perfumes.find((p) => p.id === 'hawas-for-him') || perfumes[0];
  const pBlack = perfumes.find((p) => p.id === 'hawas-black') || perfumes[1];
  const pFire = perfumes.find((p) => p.id === 'hawas-fire') || perfumes[2];

  const featuredCards = [
    {
      category: 'SIGNATURE FRAGRANCES FOR WOMEN',
      title: pForHim ? pForHim.name : 'Signature for Him by Rasasi',
      priceUSD: pForHim ? pForHim.priceUSD : 35,
      originalPriceUSD: pForHim ? pForHim.originalPriceUSD : 40,
      savingsUSD: pForHim ? pForHim.originalPriceUSD - pForHim.priceUSD : 5,
      image: pForHim ? pForHim.image : '',
      product: pForHim || perfumes[0],
    },
    {
      category: 'SIGNATURE FRAGRANCES FOR WOMEN',
      title: pBlack ? pBlack.name : 'Signature Black by Rasasi',
      priceUSD: pBlack ? pBlack.priceUSD : 35,
      originalPriceUSD: pBlack ? pBlack.originalPriceUSD : 40,
    ...
    {
      category: 'SIGNATURE FRAGRANCES FOR WOMEN',
      title: pFire ? pFire.name : 'Signature Fire by Rasasi',
      priceUSD: pFire ? pFire.priceUSD : 35,
      originalPriceUSD: pFire ? pFire.originalPriceUSD : 40,
      savingsUSD: pFire ? pFire.originalPriceUSD - pFire.priceUSD : 5,
      image: pFire ? pFire.image : '',
      product: pFire || perfumes[0],
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 items-center">
          ...
          <div className="lg:col-span-7 space-y-4 sm:space-y-6 text-center lg:text-left">
            ...
            {/* Main Headline for Signature Landing Page */}
            <h1 className="text-2xl sm:text-4xl lg:text-6xl font-black text-slate-900 tracking-tight leading-tight sm:leading-none font-sans">
              Signature Fragrances for Women,{' '}
            ...
            <p className="text-xs sm:text-base text-slate-600 leading-relaxed max-w-2xl mx-auto lg:mx-0 font-normal">
              Experience the legendary Lattafa Signature Women collection — Signature for Him, Signature Ice, Signature Black, Signature Fire, and Signature Pink. 100% original imported fragrances delivered fast across Juba.
            </p>
