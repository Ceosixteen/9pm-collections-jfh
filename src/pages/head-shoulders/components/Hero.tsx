...
export const Hero: React.FC<HeroProps> = ({
  ...
  // Select top featured items from the Head & Shoulders Collection for the right stack
  const pVitC = perfumes.find((p) => p.id === 'medix-vitc-turmeric-lotion') || perfumes[0];
  const pArgan = perfumes.find((p) => p.id === 'medix-argan-vite-cream') || perfumes[1];
  const pRetinol = perfumes.find((p) => p.id === 'medix-retinol-ferulic-cream') || perfumes[2];

  const featuredCards = [
    {
      category: 'HEAD & SHOULDERS COLLECTION',
      title: pVitC ? pVitC.name : 'Head & Shoulders Vitamin C + Turmeric Body Lotion',
      priceUSD: pVitC ? pVitC.priceUSD : 25,
      originalPriceUSD: pVitC ? pVitC.originalPriceUSD : 30,
      savingsUSD: pVitC ? pVitC.originalPriceUSD - pVitC.priceUSD : 5,
      image: pVitC ? pVitC.image : '',
      product: pVitC || perfumes[0],
    },
    {
      category: 'HEAD & SHOULDERS COLLECTION',
      title: pArgan ? pArgan.name : 'Head & Shoulders Argan Oil + Vitamin E Body Cream',
      priceUSD: pArgan ? pArgan.priceUSD : 25,
      originalPriceUSD: pArgan ? pArgan.originalPriceUSD : 30,
      savingsUSD: pArgan ? pArgan.originalPriceUSD - pArgan.priceUSD : 5,
      image: pArgan ? pArgan.image : '',
      product: pArgan || perfumes[0],
    },
    {
      category: 'HEAD & SHOULDERS COLLECTION',
      title: pRetinol ? pRetinol.name : 'Head & Shoulders Retinol + Ferulic Acid Age Rewind Body Cream',
      priceUSD: pRetinol ? pRetinol.priceUSD : 25,
      originalPriceUSD: pRetinol ? pRetinol.originalPriceUSD : 30,
      savingsUSD: pRetinol ? pRetinol.originalPriceUSD - pRetinol.priceUSD : 5,
      image: pRetinol ? pRetinol.image : '',
      product: pRetinol || perfumes[0],
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 items-center">
          ...
          <div className="lg:col-span-7 space-y-4 sm:space-y-6 text-center lg:text-left">
            ...
            {/* Main Headline for Head & Shoulders Landing Page */}
            <h1 className="text-2xl sm:text-4xl lg:text-6xl font-black text-slate-900 tracking-tight leading-tight sm:leading-none font-sans">
              Head & Shoulders Anti-Dandruff Collection,{' '}
            ...
            <p className="text-xs sm:text-base text-slate-600 leading-relaxed max-w-2xl mx-auto lg:mx-0 font-normal">
              Dermatologist-tested, active-infused anti-dandruff shampoos — Vitamin C, Argan Oil, Retinol, and Hyaluronic Acid formulas that target texture, tone, and aging from the neck down. 100% original, delivered fast across Juba.
            </p>
