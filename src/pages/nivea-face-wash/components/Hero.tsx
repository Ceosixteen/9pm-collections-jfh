...
export const Hero: React.FC<HeroProps> = ({
  ...
  // Select top featured items from the Nivea Men Face Wash Collection for the right stack
  const pDarkSpot = perfumes[0];
  const pDeepImpact = perfumes[1];
  const pAllIn1 = perfumes[2];

  const featuredCards = [
    {
      category: 'NIVEA MEN FACE WASH COLLECTION',
      title: pDarkSpot ? pDarkSpot.name : 'Nivea Men Dark Spot Reduction Face Wash',
      priceUSD: pDarkSpot ? pDarkSpot.priceUSD : 15,
      originalPriceUSD: pDarkSpot ? pDarkSpot.originalPriceUSD : 20,
      savingsUSD: pDarkSpot ? pDarkSpot.originalPriceUSD - pDarkSpot.priceUSD || 5 : 5,
      image: pDarkSpot ? pDarkSpot.image : '',
      product: pDarkSpot || perfumes[0],
    },
    {
      category: 'NIVEA MEN FACE WASH COLLECTION',
      title: pDeepImpact ? pDeepImpact.name : 'Nivea Men Deep Impact Face Wash',
      priceUSD: pDeepImpact ? pDeepImpact.priceUSD : 15,
      originalPriceUSD: pDeepImpact ? pDeepImpact.originalPriceUSD : 20,
      savingsUSD: pDeepImpact ? pDeepImpact.originalPriceUSD - pDeepImpact.priceUSD || 5 : 5,
      image: pDeepImpact ? pDeepImpact.image : '',
      product: pDeepImpact || perfumes[0],
    },
    {
      category: 'NIVEA MEN FACE WASH COLLECTION',
      title: pAllIn1 ? pAllIn1.name : 'Nivea Men All-In-1 Charcoal Face Wash',
      priceUSD: pAllIn1 ? pAllIn1.priceUSD : 15,
      originalPriceUSD: pAllIn1 ? pAllIn1.originalPriceUSD : 20,
      savingsUSD: pAllIn1 ? pAllIn1.originalPriceUSD - pAllIn1.priceUSD || 5 : 5,
      image: pAllIn1 ? pAllIn1.image : '',
      product: pAllIn1 || perfumes[0],
    },
  ];
          ...
          <div className="lg:col-span-7 space-y-4 sm:space-y-6 text-center lg:text-left">
            ...
            <div className="inline-flex flex-wrap items-center justify-center lg:justify-start gap-1.5 sm:gap-2.5 px-3 py-1.5 sm:px-4 sm:py-2.5 rounded-xl sm:rounded-2xl bg-gradient-to-r from-red-600 via-rose-600 to-purple-600 text-white font-extrabold text-[10px] sm:text-xs shadow-md border border-red-400/40">
              ...
              <span className="text-white font-bold">
                · Only <strong className="underline decoration-yellow-300 text-yellow-200 font-extrabold">9 Left</strong> in Juba!
              </span>
            ...
            {/* Main Headline for Nivea Men Face Wash Landing Page */}
            <h1 className="text-2xl sm:text-4xl lg:text-6xl font-black text-slate-900 tracking-tight leading-tight sm:leading-none font-sans">
              Nivea Men Face Wash Collection,{' '}
              <span className="inline sm:block text-[#B24BF3] font-black">
                total grooming & advanced skin defense.
              </span>
            </h1>

            {/* Subtitle */}
            <p className="text-xs sm:text-base text-slate-600 leading-relaxed max-w-2xl mx-auto lg:mx-0 font-normal">
              Dermatologist-trusted, active-charcoal and whitening formulas — Dark Spot Reduction, Deep Impact Charcoal, and All-In-1 Charcoal face washes that fight dirt, oil, and breakouts head-on. 100% original, delivered fast across Juba.
            </p>
