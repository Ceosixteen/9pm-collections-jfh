  ...
  const handleWhatsAppOrder = () => {
    ...
    const itemsSummary = items
      .map((it) => paymentCurrency === 'SSP'
        ? `• ${it.quantity}x ${it.product.name} (SSP ${(it.unitPriceUSD * 8000).toLocaleString()}/item)`
        : `• ${it.quantity}x ${it.product.name} ($${it.unitPriceUSD}/item)`
      )
      .join('%0A');
    ...
    const message = `Hello Juba Fashion Hub! I would like to place a body care order:%0A%0A` +
          ...
          <div className="p-4 sm:p-5 bg-slate-900 text-white flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full bg-[#B24BF3] text-white text-xs font-bold">
                {totalBottleCount} {totalBottleCount === 1 ? 'item' : 'items'}
              </span>
            </div>
            ...
            {totalBottleCount >= 2 ? (
              <div className="p-3.5 rounded-2xl bg-purple-100 border border-purple-200 text-xs text-purple-900 flex items-center gap-2 shadow-xs">
                ...
                <div>
                  ...
                  <span>You saved <strong>{formatMoney(bundleDiscountUSD, bundleDiscountSSP)}</strong> (-$5 off per item)!</span>
                </div>
              </div>
            ) : totalBottleCount === 1 ? (
              <div className="p-3.5 rounded-2xl bg-white border border-gray-200 text-xs text-slate-600 flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-[#B24BF3] shrink-0" />
                <div>
                  <strong className="block text-slate-900 font-bold">💡 Add 1 More Item to Save $10!</strong>
                  <span>Bundle 2 or more products to automatically subtract -$5 USD off every item.</span>
                </div>
              </div>
            ) : null}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-500 truncate">
                        {item.product.fragranceFamily || 'Face Care'} · {item.product.volume}
                      </p>
                ...
                <div className="p-4 rounded-2xl bg-white border border-gray-200 space-y-2 text-xs shadow-xs">
                  ...
                  {bundleDiscountUSD > 0 && (
                    <div className="flex justify-between font-bold text-emerald-600">
                      <span>Bundle Savings (-$5/item):</span>
                      <span>
                        -{paymentCurrency === 'SSP'
