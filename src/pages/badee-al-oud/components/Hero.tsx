...
export const Hero: React.FC<HeroProps> = ({
  ...
  // Select top featured items from the Lattafa Bade'e Al Oud Collection for the right stack
  const pBlack = perfumes.find((p) => p.id === 'oud-black') || perfumes[0];
  const pAmethyst = perfumes.find((p) => p.id === 'oud-amethyst') || perfumes[1];
  const pSublime = perfumes.find((p) => p.id === 'oud-sublime') || perfumes[2];

  const featuredCards = [
    {
      category: "BADE'E AL OUD COLLECTION",
      title: pBlack ? pBlack.name : "Bade'e Al Oud Black",
      priceUSD: pBlack ? pBlack.priceUSD : 40,
      originalPriceUSD: pBlack ? pBlack.originalPriceUSD : 45,
      savingsUSD: pBlack ? pBlack.originalPriceUSD - pBlack.priceUSD : 5,
      image: pBlack ? pBlack.image : '',
      product: pBlack || perfumes[0],
    },
    {
      category: "BADE'E AL OUD COLLECTION",
      title: pAmethyst ? pAmethyst.name : "Bade'e Al Oud Amethyst",
      priceUSD: pAmethyst ? pAmethyst.priceUSD : 40,
      originalPriceUSD: pAmethyst ? pAmethyst.originalPriceUSD : 45,
      savingsUSD: pAmethyst ? pAmethyst.originalPriceUSD - pAmethyst.priceUSD : 5,
      image: pAmethyst ? pAmethyst.image : '',
      product: pAmethyst || perfumes[0],
    },
    {
      category: "BADE'E AL OUD COLLECTION",
      title: pSublime ? pSublime.name : "Bade'e Al Oud Sublime",
      priceUSD: pSublime ? pSublime.priceUSD : 40,
      originalPriceUSD: pSublime ? pSublime.originalPriceUSD : 45,
      savingsUSD: pSublime ? pSublime.originalPriceUSD - pSublime.priceUSD : 5,
      image: pSublime ? pSublime.image : '',
      product: pSublime || perfumes[0],
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 items-center">
          ...
          <div className="lg:col-span-7 space-y-4 sm:space-y-6 text-center lg:text-left">
            ...
            {/* Main Headline for Bade'e Al Oud Landing Page */}
            <h1 className="text-2xl sm:text-4xl lg:text-6xl font-black text-slate-900 tracking-tight leading-tight sm:leading-none font-sans">
              ...
              <span className="inline sm:block text-[#B24BF3] font-black">
                majestic Eastern luxury.
              </span>
            </h1>

            {/* Subtitle */}
            <p className="text-xs sm:text-base text-slate-600 leading-relaxed max-w-2xl mx-auto lg:mx-0 font-normal">
              Experience the majestic Lattafa Bade'e Al Oud collection — Black, Amethyst, Sublime, and Noble Blush. 100% original imported fragrances delivered fast across Juba.
            </p>
