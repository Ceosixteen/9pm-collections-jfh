...
export const Hero: React.FC<HeroProps> = ({
  ...
  // Select top featured items from the 9PM Afnan Collection for the right stack
  const pRebel = perfumes.find((p) => p.id === '9pm-rebel') || perfumes[0];
  const pElixir = perfumes.find((p) => p.id === '9pm-elixir') || perfumes[1];
  const pClassic = perfumes.find((p) => p.id === '9pm-normal') || perfumes[2];

  const featuredCards = [
    {
      category: 'AFNAN 9PM COLLECTION',
      title: pRebel ? pRebel.name : '9PM Rebel by Afnan',
      priceUSD: pRebel ? pRebel.priceUSD : 40,
      originalPriceUSD: pRebel ? pRebel.originalPriceUSD : 50,
      savingsUSD: pRebel ? pRebel.originalPriceUSD - pRebel.priceUSD : 10,
      image: pRebel ? pRebel.image : '',
      product: pRebel || perfumes[0],
    },
    {
      category: 'AFNAN 9PM COLLECTION',
      title: pElixir ? pElixir.name : '9PM Elixir by Afnan',
      priceUSD: pElixir ? pElixir.priceUSD : 40,
      originalPriceUSD: pElixir ? pElixir.originalPriceUSD : 50,
      savingsUSD: pElixir ? pElixir.originalPriceUSD - pElixir.priceUSD : 10,
      image: pElixir ? pElixir.image : '',
      product: pElixir || perfumes[0],
    },
    {
      category: 'AFNAN 9PM COLLECTION',
      title: pClassic ? pClassic.name : '9PM Classic by Afnan',
      priceUSD: pClassic ? pClassic.priceUSD : 35,
      originalPriceUSD: pClassic ? pClassic.originalPriceUSD : 45,
      savingsUSD: pClassic ? pClassic.originalPriceUSD - pClassic.priceUSD : 10,
      image: pClassic ? pClassic.image : '',
      product: pClassic || perfumes[0],
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 items-center">
          ...
          <div className="lg:col-span-7 space-y-4 sm:space-y-6 text-center lg:text-left">
            ...
            {/* Main Headline for 9PM Landing Page */}
            ...
            <p className="text-xs sm:text-base text-slate-600 leading-relaxed max-w-2xl mx-auto lg:mx-0 font-normal">
              Experience the legendary Afnan 9PM collection — 9PM Rebel, 9PM Elixir, 9PM Classic, 9AM Dive, and 9PM Pour Femme. 100% original imported fragrances delivered fast across Juba.
            </p>
